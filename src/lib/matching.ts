...
export function sortFeed<T extends { matchScore: number; freshnessRank: number; verificationRank: number; sourceReliability?: number; matchProbability?: number | null }>(items: T[]) {
  return [...items].sort((a, b) => {
    if (b.matchScore !== a.matchScore) return b.matchScore - a.matchScore;
    if ((b.matchProbability ?? -1) !== (a.matchProbability ?? -1)) {
      return (b.matchProbability ?? -1) - (a.matchProbability ?? -1);
    }
